  ...
  <main class="relative z-10 w-full max-w-[1200px] px-6 py-12 flex flex-col items-center justify-center lg:flex-row gap-8 lg:gap-16 sm:px-8 md:px-12">
    <div class="w-full lg:w-1/2 space-y-8 text-center lg:text-left">
      <div class="inline-flex items-center gap-3">
        <div class="w-12 h-12 rounded-xl bg-primary-container flex items-center justify-center overflow-hidden">
          ...
          <form class="space-y-6" @submit.prevent="handleLogin">
            <div class="space-y-4">
              <div class="space-y-2">
                ...
                <input v-model="username" type="text" required placeholder="admin" class="w-full h-16 bg-on-surface/5 dark:bg-on-surface/5 border border-on-surface/8 dark:border-on-surface/10 rounded-2xl pl-6 pr-6 text-on-surface dark:text-on-surface focus:outline-none focus:ring-2 focus:ring-primary/40 transition-all  placeholder:text-on-surface-variant dark:placeholder:text-on-surface-variant" />
              </div>
              <div class="space-y-2">
                ...
                <input v-model="password" type="password" required placeholder="••••••••" class="w-full h-16 bg-on-surface/5 dark:bg-on-surface/5 border border-on-surface/8 dark:border-on-surface/10 rounded-2xl pl-6 pr-6 text-on-surface dark:text-on-surface focus:outline-none focus:ring-2 focus:ring-primary/40 transition-all  placeholder:text-on-surface-variant dark:placeholder:text-on-surface-variant" />
              </div>
            </div>
            <p v-if="authStore.error" class="text-error dark:text-error text-sm text-center">{{ authStore.error }}</p>
            <button :disabled="authStore.isLoading" class="group relative w-full h-16 bg-primary hover:bg-primary-dim rounded-xl font-bold text-on-primary uppercase tracking-wider active:scale-[0.98] transition-colors disabled:opacity-50 disabled:cursor-not-allowed" type="submit">
