...
class="flex items-center gap-2 px-5 py-3 bg-gradient-to-br from-orange-500 to-orange-700 hover:scale-[1.02] text-white rounded-2xl font-black text-sm transition-all active:scale-95 shadow-lg shadow-orange-900/30 focus:outline-none focus:ring-2 focus:ring-orange-500/50"
        ...
        <div class="grid grid-cols-3 gap-4">
          ...
          <div class="col-span-2 space-y-4">
            ...
            <!-- Upcoming Sessions -->
            <div class="glass-panel bg-surface-container-low/60 border border-white/5 shadow-2xl rounded-3xl overflow-hidden">
              ...
              <div v-if="scheduleStore.isLoading" class="p-6 space-y-3">
                <div v-for="i in 3" :key="i" class="h-20 rounded-xl bg-surface-container-highest animate-pulse" />
              </div>
...
class="mt-4 px-5 py-2.5 bg-gradient-to-br from-orange-500 to-orange-700 hover:scale-[1.02] text-white rounded-xl font-bold text-sm transition-all active:scale-95"
              ...
              <div v-else class="divide-y divide-zinc-800/50">
                <div
v-for="session in mySessions" :key="session.id"
...
class="w-12 h-12 rounded-xl mx-auto flex flex-col items-center justify-center"
                      :class="session.status === 'completed' ? 'bg-surface-container-highest' : 'bg-orange-600'">
                      <p
...
type="file" accept="image/*" class="hidden"
            ...
            <div class="glass-panel bg-surface-container-low/60 border border-white/5 shadow-2xl rounded-3xl overflow-hidden">
              ...
              <div class="p-4 grid grid-cols-2 gap-3">
                <!-- Upload area -->
                <label class="border-2 border-dashed border-white/10 rounded-xl p-6 flex flex-col items-center justify-center gap-2 cursor-pointer hover:border-orange-500/50 transition-colors text-center group">
                  ...
                  <input type="file" accept="image/*" class="hidden" aria-label="Upload session photo proof" @change="handleGenericProofUpload" />
                </label>

                <!-- Pending homework -->
                <div v-if="pendingHomework" class="bg-surface-container-highest/50 border border-white/10 rounded-xl p-4">
...
class="mt-2 text-xs text-orange-500 hover:text-orange-400 font-bold flex items-center gap-1 transition-colors focus:outline-none focus:underline"
                ...
                <div v-else class="bg-surface-container-highest/30 border border-dashed border-white/10 rounded-xl p-4 flex items-center justify-center">
          ...
          <div class="space-y-4">
            <!-- Sessions used meter -->
            <div class="glass-panel bg-surface-container-low/60 border border-white/5 shadow-2xl rounded-3xl p-5">
              ...
              <div class="relative flex items-center justify-center mb-4">
                <svg class="w-32 h-32 -rotate-90" viewBox="0 0 100 100" aria-hidden="true">
                  ...
                  <circle
cx="50" cy="50" r="40" fill="none" stroke="#f97316" stroke-width="8"
              ...
              <button class="w-full mt-4 py-2.5 rounded-xl border border-white/10 text-zinc-400 hover:text-white hover:border-zinc-600 text-xs font-semibold transition-all focus:outline-none focus:ring-2 focus:ring-zinc-500/50">
                Manage Subscription
              </button>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3">
              <div class="glass-panel bg-surface-container-low/60 border border-white/5 shadow-2xl rounded-3xl p-4 text-center">
...
class="text-zinc-500 hover:text-white transition-colors focus:outline-none focus:ring-2 focus:ring-zinc-500 rounded-lg p-1"
          ...
          <form class="space-y-4" @submit.prevent="submitRequest">
            <div>
              ...
              <select
id="req-teacher" v-model="requestForm.teacherId" required
            ...
            <div>
              ...
              <input
id="req-date" v-model="requestForm.startTime" type="datetime-local" required
                class="w-full bg-surface-container-highest border border-white/10 text-white rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500/50" />
            </div>
