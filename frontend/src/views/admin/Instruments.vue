...
onMounted(async () => {
  await Promise.all([shopStore.fetchProducts(), shopStore.fetchAllOrders()])
})
...
const saveProduct = async function () {
  try {
    const payload = {
      ...
      price_cents: Math.round(displayPricePHP.value * 100),
    }
  ...
  <div class="min-h-screen pb-20">
    ...
    <div v-if="activeTab === 'catalog'" class="grid grid-cols-1 gap-4">
      <div
        ...
        :key="product.id"
        class="glass-thin rounded-3xl p-6 border border-on-surface/5 flex flex-col md:flex-row items-center gap-6"
      >
        <div
          class="size-24 rounded-2xl bg-on-surface/20 dark:bg-on-surface/40 overflow-hidden shrink-0 border border-on-surface/5"
        >
          <img
        ...
        <div class="flex-1 min-w-0 text-center md:text-left">
          ...
          <p class="text-sm font-bold text-on-surface-variant line-clamp-1">
            {{ product.description }}
          </p>
          <div class="flex flex-wrap justify-center md:justify-start gap-4 mt-2">
            ...
            <span
              ...
              :class="
                product.stock <= 3
                  ? 'text-error font-extrabold animate-pulse'
                  : 'text-on-surface/40'
              "
            >
            ...
            <span
              :class="product.isActive ? 'text-success' : 'text-error'"
              class="text-xs font-semibold uppercase"
            >
        ...
        <div class="flex gap-3">
          <label
            class="h-12 w-12 rounded-2xl bg-on-surface/5 text-on-surface-variant hover:text-on-surface transition-all flex items-center justify-center cursor-pointer"
          >
            <input
              type="file"
              class="hidden"
          ...
          <button
            ...
            class="h-12 w-12 rounded-2xl bg-on-surface/5 text-on-surface-variant hover:text-on-surface transition-all flex items-center justify-center"
          >
            <span class="material-symbols-outlined">edit</span>
          </button>
          <button
            @click="shopStore.deleteProduct(product.id)"
            class="h-12 w-12 rounded-2xl bg-on-surface/5 text-on-surface-variant hover:text-error transition-all flex items-center justify-center"
          >
    ...
    <div v-else class="space-y-4">
      ...
      <div
        class="flex flex-wrap gap-2 mb-6 bg-on-surface/5 p-1.5 rounded-2xl border border-on-surface/5 max-w-max"
      >
        <button
      ...
      <div
        ...
        class="glass-thin rounded-3xl p-6 border border-on-surface/5 hover:border-primary/30 transition-all cursor-pointer group flex flex-col md:flex-row md:items-center justify-between gap-4"
      >
        <div class="flex items-center gap-4">
          <div
        ...
        <div class="flex items-center gap-6">
          ...
          <span
            class="material-symbols-outlined text-on-surface/10 group-hover:text-primary transition-colors"
            >chevron_right</span
          >
        </div>
      </div>
    ...
    <div
      v-if="isEditorOpen && selectedProduct"
    ...
    >
      ...
      ></div>
      <div
        class="relative w-full max-w-lg glass-thin rounded-[3rem] border border-on-surface/10 p-10"
      >
        <h2 class="text-3xl font-semibold text-on-surface mb-8">
          {{ selectedProduct?.id ? 'Edit' : 'New' }} Product
        </h2>

        <div class="space-y-6">
          <div>
            <label class="text-xs font-semibold text-on-surface-variant uppercase mb-2 block"
              >Product Image</label
            >
            <div
            ...
            >
              ...
              <div
                v-else
                class="flex flex-col items-center text-on-surface/40 group-hover:text-on-surface/60 transition-colors"
              >
        ...
        <div class="mt-10 flex gap-4">
          <button
            @click="isEditorOpen = false"
            class="flex-1 h-14 rounded-2xl bg-on-surface/5 text-on-surface font-semibold uppercase text-xs hover:bg-on-surface/10 transition-all"
          >
            Cancel
          </button>
          <button
            @click="saveProduct"
            class="flex-1 h-14 rounded-2xl bg-primary text-on-primary font-semibold uppercase text-xs hover:bg-primary-dim transition-all shadow-xl"
          >
            Save Product
          </button>
        </div>
    ...
    <!-- Order Detail Modal -->
    <div
      v-if="isOrderDetailOpen && selectedOrder"
      class="fixed inset-0 z-50 flex items-center justify-center p-6"
    >
      <div
        class="absolute inset-0 bg-black/40 dark:bg-black/70"
        @click="isOrderDetailOpen = false"
      ></div>
      <div
        class="relative w-full max-w-2xl glass-thin rounded-[3rem] border border-on-surface/10 p-10 max-h-[90vh] overflow-y-auto scrollbar-hide"
      >
        ...
        <div class="space-y-8">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-8 border-b border-on-surface/5 pb-6">
            <div>
              ...
              <p class="font-semibold text-on-surface">
                {{ selectedOrder.user?.name }} ({{ selectedOrder.user?.role }})
              </p>
              <p class="text-xs font-bold text-on-surface-variant">
                {{ selectedOrder.user?.email }}
              </p>
              <p
                v-if="selectedOrder.user?.contactNumber"
                class="text-xs font-bold text-on-surface-variant mt-1"
              >
                Phone: {{ selectedOrder.user?.contactNumber }}
              </p>
            </div>
            <div>
              <p class="text-xs font-semibold text-on-surface-variant uppercase mb-1">
              ...
              <p class="text-xs font-bold text-on-surface-variant mt-1">
          ...
          <div
            v-if="selectedOrder.user?.parentName || selectedOrder.user?.parentContact"
            class="grid grid-cols-1 md:grid-cols-2 gap-8 border-b border-on-surface/5 pb-6"
          >
            ...
            <div v-if="selectedOrder.user?.parentContact">
              ...
              <p class="text-sm font-bold text-on-surface">
                {{ selectedOrder.user?.parentContact }}
              </p>
            </div>
          </div>
          ...
          <div>
            ...
            <div class="space-y-3">
              <div
                ...
                :key="item.id"
                class="flex items-center justify-between p-4 rounded-2xl bg-on-surface/5 border border-on-surface/5"
              >
                <div class="flex items-center gap-4">
                  <div
                    class="size-10 rounded-xl bg-on-surface/20 dark:bg-on-surface/40 overflow-hidden"
                  >
                    <img
                  ...
                  <div>
                    ...
                    <p class="text-xs font-bold text-on-surface-variant uppercase">
                      {{ item.quantity }} units @ {{ formatPrice(item.priceCentsAtPurchase) }}
                    </p>
                  </div>
                </div>
                <p class="font-semibold text-on-surface">
            ...
            <div
              class="mt-6 flex justify-between items-center p-6 rounded-3xl bg-primary/10 border border-primary/20"
            >
          ...
          <div v-if="selectedOrder.notes">
            ...
            <p class="p-4 rounded-2xl bg-on-surface/5 text-xs font-bold text-on-surface italic">
              "{{ selectedOrder.notes }}"
            </p>
          </div>
          ...
          <div v-if="selectedOrder.status === 'pending'" class="flex gap-4 pt-4">
            <button
              ...
              class="flex-1 h-14 rounded-2xl bg-error/10 text-error font-semibold uppercase text-xs hover:bg-error hover:text-on-error transition-all"
            >
              Reject
            </button>
            <button
              @click="updateOrderStatus(selectedOrder.id, 'approved')"
              class="flex-1 h-14 rounded-2xl bg-primary text-on-primary font-semibold uppercase text-xs hover:bg-primary-dim transition-all shadow-xl shadow-e2"
            >
          ...
          <div v-else-if="selectedOrder.status === 'approved'" class="pt-4">
            <button
              @click="updateOrderStatus(selectedOrder.id, 'fulfilled')"
              class="w-full h-14 rounded-2xl bg-success text-on-success font-semibold uppercase text-xs hover:bg-success transition-all shadow-xl shadow-success/20"
            >
              Mark as Fulfilled
            </button>
          </div>
