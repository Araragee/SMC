  ...
  <div class="min-h-screen pb-20">
    ...
    <div class="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
      ...
      <div class="flex items-center gap-4">
        <button
          @click="openEditor()"
          class="h-16 px-8 bg-white text-black rounded-3xl font-semibold text-sm uppercase transition-all hover:scale-105 active:scale-95 shadow-xl shadow-white/5 flex items-center gap-3"
        >
    ...
    <div v-if="activeTab === 'catalog'" class="grid grid-cols-1 gap-4">
      <div v-for="product in shopStore.products" :key="product.id" class="glass-thin rounded-3xl p-6 border border-white/5 flex flex-col md:flex-row items-center gap-6">
        <div class="size-24 rounded-2xl bg-black/20 dark:bg-black/40 overflow-hidden shrink-0 border border-white/5">
        ...
        <div class="flex-1 min-w-0 text-center md:text-left">
          ...
          <div class="flex flex-wrap justify-center md:justify-start gap-4 mt-2">
            <span class="text-xs font-semibold text-orange-500 uppercase">{{ formatPrice(product.priceCents) }}</span>
            <span class="text-xs font-semibold uppercase" :class="product.stock <= 3 ? 'text-rose-500 font-extrabold animate-pulse' : 'text-white/40'">
    ...
    <div v-else class="space-y-4">
      ...
      <div v-for="order in filteredOrders" :key="order.id" @click="viewOrder(order)" class="glass-thin rounded-3xl p-6 border border-white/5 hover:border-orange-500/30 transition-all cursor-pointer group flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div class="flex items-center gap-4">
    ...
    <div v-if="isEditorOpen && selectedProduct" class="fixed inset-0 z-50 flex items-center justify-center p-6">
      ...
      <div class="relative w-full max-w-lg glass-thin rounded-[3rem] border border-white/10 p-10">
        ...
        <div class="mt-10 flex gap-4">
          <button @click="isEditorOpen = false" class="flex-1 h-14 rounded-2xl bg-white/5 text-on-surface font-semibold uppercase text-xs hover:bg-white/10 transition-all">Cancel</button>
          <button @click="saveProduct" class="flex-1 h-14 rounded-2xl bg-orange-500 text-white font-semibold uppercase text-xs hover:bg-orange-600 transition-all shadow-xl shadow-orange-500/20">Save Product</button>
        </div>
      </div>
    </div>
      ...
      <div class="relative w-full max-w-2xl glass-thin rounded-[3rem] border border-white/10 p-10 max-h-[90vh] overflow-y-auto scrollbar-hide">
        ...
        <div class="space-y-8">
          ...
          <div>
            ...
            <div class="space-y-3">
              <div v-for="item in selectedOrder.items" :key="item.id" class="flex items-center justify-between p-4 rounded-2xl bg-white/5 border border-white/5">
                <div class="flex items-center gap-4">
                  <div class="size-10 rounded-xl bg-black/20 dark:bg-black/40 overflow-hidden">
            ...
            <div class="mt-6 flex justify-between items-center p-6 rounded-3xl bg-orange-500/10 border border-orange-500/20">
              <span class="font-semibold text-orange-500 uppercase text-xs">Total Amount</span>
          ...
          <div v-if="selectedOrder.status === 'pending'" class="flex gap-4 pt-4">
            <button @click="updateOrderStatus(selectedOrder.id, 'rejected')" class="flex-1 h-14 rounded-2xl bg-rose-500/10 text-rose-500 font-semibold uppercase text-xs hover:bg-rose-500 hover:text-white transition-all">Reject</button>
            <button @click="updateOrderStatus(selectedOrder.id, 'approved')" class="flex-1 h-14 rounded-2xl bg-orange-500 text-white font-semibold uppercase text-xs hover:bg-orange-600 transition-all shadow-xl shadow-orange-500/20">Approve & Deduct Stock</button>
          </div>
          <div v-else-if="selectedOrder.status === 'approved'" class="pt-4">
            <button @click="updateOrderStatus(selectedOrder.id, 'fulfilled')" class="w-full h-14 rounded-2xl bg-emerald-500 text-white font-semibold uppercase text-xs hover:bg-emerald-600 transition-all shadow-xl shadow-emerald-500/20">Mark as Fulfilled</button>
          </div>
        </div>
      </div>
    </div>
