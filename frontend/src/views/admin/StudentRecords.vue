...
const filteredSessions = computed(() => {
  return scheduleStore
    ...
    .filter((s: Session) => {
      ...
      if (filters.value.instrument !== 'all' && s.instrumentId !== Number(filters.value.instrument))
        return false
      return true
    })
...
const totalUsed = computed(() => {
  return scheduleStore
    .getSessionsByStudentId(studentId)
    .filter((s: Session) => s.status === 'completed').length
})
...
const handleDeleteEnrollment = async (enrollment: Enrollment) => {
  const teacherName =
    usersStore.users.find((u: User) => u.id === enrollment.teacherId)?.name ||
...
<template>
  <div class="space-y-8 max-w-[1600px] mx-auto pb-12 px-4 sm:px-6">
    ...
    <section v-if="student" class="flex flex-col md:flex-row gap-6 items-start">
      <BaseCard
        class="flex-1 p-6 liquid-glass border border-on-surface/10 relative overflow-hidden group"
      >
        <div class="flex items-center gap-6">
          <div
            class="size-24 rounded-full bg-surface-container-highest border border-outline-variant flex items-center justify-center overflow-hidden flex-shrink-0 relative"
          >
            <img
            ...
            />
            <span v-else class="material-symbols-outlined text-4xl text-on-surface-variant"
              >person</span
            >
          </div>
          <div>
            <h1 class="text-3xl font-semibold text-on-surface tracking-tight">
              {{ student.name }}
            </h1>
            ...
            <div class="flex flex-wrap gap-2 mt-3">
              <span
                v-for="inst in student.instruments"
                :key="inst.id"
                class="px-2 py-1 rounded-md bg-on-surface/5 border border-on-surface/10 text-xs font-bold text-on-surface/80 uppercase"
              >
                {{ inst.name }}
              </span>
            </div>
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-xs text-on-surface/50 mt-0.5">
                {{ enrollment.sessionsUsed }} used ·
                {{ enrollment.sessionsPurchased - enrollment.sessionsUsed }} remaining of
                {{ enrollment.sessionsPurchased }} purchased
              </p>
            </div>
          </div>
          <button
      ...
      <div class="flex gap-4 w-full sm:w-auto">
        ...
        <BaseDropdown
          ...
          :options="[
            ...
            ...usersStore.instruments.map((inst) => ({ value: inst.id, label: inst.name })),
          ]"
        />
      </div>
    ...
    <section>
      ...
      <div
        ...
        class="text-center p-12 border border-dashed border-on-surface/20 rounded-2xl text-on-surface/50"
      >
      ...
      <div v-else class="space-y-4">
        <BaseCard
          ...
          :key="session.id"
          class="p-4 liquid-glass border border-on-surface/10 hover:border-on-surface/20 transition-colors"
        >
          <div class="flex flex-col md:flex-row gap-4 justify-between md:items-center">
            <div class="flex items-start gap-4">
              <div
                class="size-12 rounded-xl bg-on-surface/5 flex items-center justify-center shrink-0 border border-on-surface/10"
              >
                <span
              ...
              <div>
                <div class="flex items-center gap-2 mb-1">
                  ...
                  <span
                    v-if="session.isManualEntry"
                    class="px-2 py-0.5 rounded text-xs uppercase font-bold bg-primary/20 text-primary border border-primary/30"
                    >Paper Record</span
                  >
                  <span v-if="session.sessionNumber" class="text-xs text-on-surface/40"
                ...
                <div class="text-sm text-on-surface/60 flex flex-wrap gap-x-4 gap-y-1 items-center">
                  <span class="flex items-center gap-1"
                    ...
                    {{ getTeacherName(session.teacherId) }}</span
                  >
                  <span class="flex items-center gap-1"
                    ><span class="material-symbols-outlined text-[14px]">music_note</span>
                    {{ getInstrumentName(session.instrumentId) }}</span
                  >
                </div>

                <p
                  v-if="session.notes"
                  class="text-sm text-on-surface/80 mt-3 pl-3 border-l-2 border-primary/30 italic"
                >
                  "{{ session.notes }}"
                </p>
              </div>
            ...
            <div class="flex items-center gap-3">
              <span
                class="text-xs font-bold uppercase px-3 py-1 rounded-full border"
                :class="{
                  ...
                  'border-on-surface/20 text-on-surface/50': !['completed', 'scheduled'].includes(
                    session.status
                  ),
                }"
              >
