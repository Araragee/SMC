...
<template>
  <div class="space-y-8 max-w-[1600px] mx-auto pb-12 px-4 sm:px-6">
    ...
    <section v-if="student" class="flex flex-col md:flex-row gap-6 items-start">
      <BaseCard class="flex-1 p-6 liquid-glass border border-on-surface/10 relative overflow-hidden group">
        <div class="flex items-center gap-6">
          ...
          <div>
            <h1 class="text-3xl font-black text-on-surface tracking-tight">{{ student.name }}</h1>
            ...
            <div class="flex flex-wrap gap-2 mt-3">
               <span v-for="inst in student.instruments" :key="inst.id" class="px-2.5 py-1 rounded-md bg-on-surface/5 border border-on-surface/10 text-xs font-bold text-on-surface/80 uppercase tracking-widest">
                 {{ inst.name }}
               </span>
            </div>
          </div>
      ...
      <BaseCard class="w-full md:w-80 p-6 liquid-glass border border-on-surface/10 shrink-0">
        <h3 class="text-sm font-black uppercase tracking-[0.2em] text-on-surface/50 mb-4">Enrollment Status</h3>
        <div class="space-y-4">
          <div>
          ...
          <button
            class="w-full mt-1 flex items-center justify-center gap-2 text-xs font-bold uppercase tracking-widest text-on-surface/50 hover:text-primary border border-on-surface/10 hover:border-primary/30 rounded-xl py-2 transition-all"
    ...
    <section v-if="interactionsStore.enrollments.length > 0">
      <h2 class="text-xs font-black uppercase tracking-[0.2em] text-on-surface/40 mb-3">Enrollments</h2>
      <div class="space-y-2">
        <BaseCard
    ...
    <section>
      ...
      <div v-else class="space-y-4">
        <BaseCard v-for="session in filteredSessions" :key="session.id" class="p-5 liquid-glass border border-on-surface/10 hover:border-on-surface/20 transition-colors">
          <div class="flex flex-col md:flex-row gap-4 justify-between md:items-center">

            <div class="flex items-start gap-4">
              <div class="w-12 h-12 rounded-xl bg-on-surface/5 flex items-center justify-center shrink-0 border border-on-surface/10">
              ...
              <div>
                <div class="flex items-center gap-2 mb-1">
                  ...
                  <span v-if="session.isManualEntry" class="px-2 py-0.5 rounded text-[10px] uppercase font-bold tracking-widest bg-primary/20 text-primary border border-primary/30">Paper Record</span>
            ...
            <div class="flex items-center gap-3">
               <span class="text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full border"
                 :class="{
                   ...
                   'border-on-surface/20 text-on-surface/50': !['completed', 'scheduled'].includes(session.status)
                 }">
                 {{ session.status }}
               </span>
            </div>
