...
onMounted(async () => {
  await Promise.all([paymentsStore.fetchPayments(), usersStore.fetchUsersByRole('student')])
})
  ...
  <div class="page">
    ...
    <header class="flex flex-col md:flex-row md:items-end justify-between gap-6 py-8">
      ...
      <div class="flex flex-wrap gap-4">
        <div
          class="glass-medium rounded-3xl px-8 py-4 border border-outline-variant/30 flex flex-col items-center min-w-[160px]"
        >
          <p class="text-xs font-semibold text-on-surface-variant uppercase mb-1">
            Total Collected
          </p>
          <p class="text-3xl font-semibold text-success">{{ formatCurrency(totalCollected) }}</p>
        </div>
        <div
          class="glass-medium rounded-3xl px-8 py-4 border border-outline-variant/30 flex flex-col items-center min-w-[160px]"
        >
          <p class="text-xs font-semibold text-on-surface-variant uppercase mb-1">Pending</p>
          <p class="text-3xl font-semibold text-warning">{{ formatCurrency(totalPending) }}</p>
        ...
        >
          ...
          <p class="text-3xl font-semibold text-primary">
            {{ formatCurrency(paymentsStore.payments.reduce((s, p) => s + p.amount, 0)) }}
          </p>
        </div>
      </div>
    </header>
    ...
    <!-- Ledger Table -->
    <section
      class="glass-heavy rounded-3xl border border-outline-variant/30 overflow-hidden shadow-2xl shadow-e2 dark:shadow-e2"
    >
      <div v-if="paymentsStore.isLoading" class="p-24 text-center">
        <span class="material-symbols-outlined text-5xl animate-spin text-primary/40"
      ...
      <div v-else-if="paymentsStore.payments.length === 0" class="py-32 text-center">
        <div
          class="size-20 rounded-full bg-surface-container-highest/50 flex items-center justify-center mx-auto mb-6"
        >
          <span class="material-symbols-outlined text-5xl text-on-surface-variant/30"
            >receipt_long</span
          >
        </div>
        <p class="text-xl font-semibold text-on-surface">No transactions found</p>
        <p class="text-on-surface-variant mt-2 font-medium">
          When students complete enrollments, they will appear here.
        </p>
      </div>
      ...
      <div v-else class="md:overflow-x-auto">
        <table class="data-table stacked">
          ...
          <tbody class="divide-y divide-outline-variant/10">
            ...
            >
              <td data-label="Date" class="text-sm font-bold text-on-surface">
                {{ formatDate(pay.date) }}
              </td>
              <td data-label="Student">
                <div class="flex items-center gap-3">
                  <div
                    class="size-8 rounded-lg bg-surface-container-highest flex items-center justify-center text-xs font-semibold"
                  >
                    {{ getStudentName(pay.student_id).charAt(0).toUpperCase() }}
                  </div>
                  <span class="text-sm font-bold text-on-surface">{{
                    getStudentName(pay.student_id)
                  }}</span>
                </div>
              </td>
              <td data-label="Method">
                <span class="text-xs font-medium text-on-surface-variant flex items-center gap-2">
                  <span class="material-symbols-outlined text-sm opacity-50">{{
                    pay.method === 'card' ? 'credit_card' : 'account_balance'
                  }}</span>
        ...
        <table class="data-table stacked">
          ...
          <tbody class="divide-y divide-outline-variant/10">
            <tr
              ...
              :key="row.label"
              class="hover:bg-primary/[0.02] transition-colors"
            >
              <td data-label="Month" class="text-sm font-bold text-on-surface">{{ row.label }}</td>
              <td
                data-label="Transactions"
                class="text-right text-sm text-on-surface-variant font-medium"
              >
                {{ row.count }}
              </td>
              <td data-label="Collected" class="text-right text-sm font-semibold text-success">
