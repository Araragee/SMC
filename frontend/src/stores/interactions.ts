...
export const useInteractionsStore = defineStore('interactions', {
  ...
  actions: {
    async fetchMyTeachers() {
      try {
        const { data } = await axios.get(`${API_URL}/enrollments/my-teachers`, {
          headers: authHeaders(),
        })
        ...
        if (session) {
          ...
          const newProof = {
            ...
            uploaderId: response.data.uploader_id
              ? Number(response.data.uploader_id)
              : auth.currentUser?.id,
    ...
    async deleteEnrollment(enrollmentId: number) {
      ...
      try {
        ...
        toast.success(
          'Enrollment removed',
          'The enrollment has been deleted and unused sessions rolled back.'
        )
      } catch (err: any) {
    ...
    async completeHomework(sessionId: number) {
      ...
      try {
        ...
        if (sessionData && sessionData.homeworks && sessionData.homeworks.length > 0) {
          ...
          await axios.put(
            `${API_URL}/homework/${homeworkId}?is_completed=true`,
            {},
            { headers: authHeaders() }
          )
        }
