  ...
  getters: {
    getNotificationsByUserId: (state) => {
      return (userId: number) =>
        state.notifications.filter((n) => n.userId === userId || n.userId === null)
    },
    unreadNotifications: (state): Notification[] => state.notifications.filter((n) => !n.isRead),
    unreadCount: (state): number => state.notifications.filter((n) => !n.isRead).length,
    getUnreadCount: (state) => {
      return (userId: number) =>
        state.notifications.filter((n) => (n.userId === userId || n.userId === null) && !n.isRead)
          .length
    },
  },
  actions: {
