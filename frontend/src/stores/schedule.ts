...
const mapSession = function (session: any): Session {
  return {
    ...
    conflictSessionId: session.conflict_session_id
      ? Number(session.conflict_session_id)
      : undefined,
  ...
  getters: {
    ...
    pendingSessions: (state): Session[] =>
      state.allSessions.filter(
        (s) => s.status === 'pending_teacher' || s.status === 'pending_admin'
      ),
  },
  actions: {
    ...
    async fetchTeacherPublicSessions(teacherId: number) {
      ...
      this.error = null
      try {
        const response = await axios.get(`${API_URL}/sessions/teacher/${teacherId}/public`, {
          headers: authHeaders(),
        })
    ...
    async proposeSessionAsStudent(data: {
      ...
      try {
        ...
        const response = await axios.post(`${API_URL}/sessions/propose/student`, payload, {
          headers: authHeaders(),
        })
        const session = mapSession(response.data)
        this._upsertSession(session)
    ...
    async proposeSessionAsTeacher(data: {
      ...
      try {
        ...
        const response = await axios.post(`${API_URL}/sessions/propose/teacher`, payload, {
          headers: authHeaders(),
        })
        const session = mapSession(response.data)
        this._upsertSession(session)
    ...
    async approveAsTeacher(sessionId: number) {
      ...
      try {
        const response = await axios.post(
          `${API_URL}/sessions/${sessionId}/approve/teacher`,
          {},
          { headers: authHeaders() }
        )
    ...
    async rejectAsTeacher(sessionId: number, notes?: string) {
      ...
      try {
        const response = await axios.post(
          `${API_URL}/sessions/${sessionId}/reject/teacher`,
    ...
    async approveAsAdmin(sessionId: number) {
      ...
      try {
        const response = await axios.post(
          `${API_URL}/sessions/${sessionId}/approve/admin`,
          {},
          { headers: authHeaders() }
        )
        this._upsertSession(mapSession(response.data))
    ...
    async rejectAsAdmin(sessionId: number, notes?: string) {
      ...
      try {
        const response = await axios.post(
          `${API_URL}/sessions/${sessionId}/reject/admin`,
          { notes, version: this._sessionVersion(sessionId) },
          { headers: authHeaders() }
        )
        this._upsertSession(mapSession(response.data))
    ...
    async counterAsTeacher(
      ...
      try {
        const response = await axios.post(
          `${API_URL}/sessions/${sessionId}/counter/teacher`,
          {
            start_time: data.startTime,
            end_time: data.endTime,
            notes: data.notes,
    ...
    async counterAsStudent(
      ...
      try {
        const response = await axios.post(
          `${API_URL}/sessions/${sessionId}/counter/student`,
          {
            start_time: data.startTime,
            end_time: data.endTime,
            notes: data.notes,
    ...
    async approveAsStudent(sessionId: number) {
      ...
      try {
        const response = await axios.post(
          `${API_URL}/sessions/${sessionId}/approve/student`,
          {},
          { headers: authHeaders() }
        )
        this._upsertSession(mapSession(response.data))
    ...
    async rejectAsStudent(sessionId: number, notes?: string) {
      ...
      try {
        const response = await axios.post(
          `${API_URL}/sessions/${sessionId}/reject/student`,
          { notes, version: this._sessionVersion(sessionId) },
          { headers: authHeaders() }
        )
        this._upsertSession(mapSession(response.data))
    ...
    async nudgeSession(sessionId: number) {
      ...
      try {
        const response = await axios.post(
          `${API_URL}/sessions/${sessionId}/nudge`,
          {},
          { headers: authHeaders() }
        )
        this._upsertSession(mapSession(response.data))
    ...
    async fetchStudentRecords(studentId: number) {
      ...
      this.error = null
      try {
        const response = await axios.get(`${API_URL}/sessions/student/${studentId}/records`, {
          headers: authHeaders(),
        })
    ...
    async bulkAction(sessionIds: number[], action: 'approve' | 'cancel' | 'complete') {
      ...
      try {
        ...
        toast.success(
          ...
          `Successfully processed ${sessionIds.length} sessions.`
        )
