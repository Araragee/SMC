...
export const useUsersStore = defineStore('users', {
  ...
  actions: {
    ...
    async fetchMyStudents() {
      const { data } = await axios.get(`${API_URL}/enrollments/my-students`, {
        headers: authHeaders(),
      })
    ...
    // Instruments are a fixed lookup table, but six views call this on every
    // mount. Cache it in the store; `force` covers the admin CRUD screens.
    async fetchInstruments(force = false) {
      if (!force && this.instruments.length > 0) return
      try {
    ...
    async updateUser(userId: number, updateData: Partial<User>) {
      ...
      try {
        ...
        if (updateData.contactNumber !== undefined)
          payload.contact_number = updateData.contactNumber
        ...
        if (updateData.parentContact !== undefined)
          payload.parent_contact = updateData.parentContact
        if (updateData.sessionsEnrolled !== undefined)
          payload.sessions_enrolled = updateData.sessionsEnrolled
        if (updateData.instruments !== undefined)
          payload.instrument_ids = updateData.instruments.map((i) => i.id)
        ...
        const response = await axios.put(`${API_URL}/users/${Number(userId)}`, payload, {
          headers: authHeaders(),
        })
