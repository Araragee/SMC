...
const getRoleColor = function (role: string) {
  ...
  if (role === 'admin') return 'text-tertiary bg-tertiary/10'
...
<template>
  <Teleport to="body">
    ...
    >
      ...
      <div
        class="modal-shell relative w-full max-w-md glass-heavy rounded-3xl p-6 shadow-2xl flex flex-col gap-4 max-h-[80vh]"
      >
        <!-- Header -->
        ...
        <!-- Group name input -->
        <input v-if="tab === 'group'" v-model="groupName" class="input" placeholder="Group name…" />

        <!-- Search -->
        <input v-model="search" class="input" placeholder="Search people…" />
        ...
        <div class="flex-1 overflow-y-auto space-y-1 max-h-48">
            ...
            :class="
              tab === 'dm'
                ? selectedDM === user.id
                  ...
                  : 'hover:bg-on-surface/[0.04] dark:hover:bg-on-surface/5'
          ...
          >
            <div
              class="size-9 rounded-xl bg-primary/10 flex items-center justify-center text-primary font-semibold text-sm shrink-0"
            >
            ...
            <span
              v-if="
                (tab === 'dm' && selectedDM === user.id) ||
                (tab === 'group' && selectedGroup.includes(user.id))
              "
          ...
          <p v-if="filtered.length === 0" class="text-center text-xs text-on-surface-variant py-4">
            No users found
          </p>
        </div>
        ...
        <button
          ...
          :disabled="
            isSubmitting ||
            (tab === 'dm' ? !selectedDM : !groupName.trim() || selectedGroup.length === 0)
          "
          @click="confirm"
        >
