    ...
    >
      ...
      <div class="relative w-full max-w-md glass-heavy rounded-3xl p-6 shadow-2xl flex flex-col gap-5 max-h-[80vh]">
        ...
        <div class="flex items-center justify-between">
          ...
          <button
            class="w-9 h-9 rounded-xl bg-on-surface/[0.04] dark:bg-on-surface/5 flex items-center justify-center text-on-surface-variant hover:text-on-surface transition-colors"
            @click="emit('close')"
          >
        ...
        <input
          v-if="tab === 'group'"
          v-model="groupName"
          class="w-full bg-on-surface/[0.04] dark:bg-on-surface/5 border border-on-surface/[0.06] dark:border-on-surface/5 rounded-2xl px-4 py-2.5 text-sm text-on-surface placeholder:text-on-surface-variant focus:outline-none focus:ring-2 focus:ring-primary/40"
        ...
        <div class="flex-1 overflow-y-auto space-y-1 max-h-48">
          <button
            ...
            :class="tab === 'dm'
              ? selectedDM === user.id ? 'bg-primary/10 border border-primary/30' : 'hover:bg-on-surface/[0.04] dark:hover:bg-on-surface/5'
              : selectedGroup.includes(user.id) ? 'bg-primary/10 border border-primary/30' : 'hover:bg-on-surface/[0.04] dark:hover:bg-on-surface/5'"
            @click="tab === 'dm' ? (selectedDM = user.id) : toggleGroup(user.id)"
          >
            <div class="w-9 h-9 rounded-xl bg-primary/10 flex items-center justify-center text-primary font-black text-sm shrink-0">
        ...
        <!-- Confirm -->
        <button
          class="w-full py-3 rounded-2xl bg-primary text-on-primary font-black text-sm uppercase tracking-widest disabled:opacity-40 disabled:cursor-not-allowed hover:opacity-90 transition-opacity active:scale-[0.98]"
