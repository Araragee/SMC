      ...
      <div class="relative w-full max-w-md glass-heavy rounded-3xl p-6 shadow-2xl flex flex-col gap-4 max-h-[80vh]">
        ...
        <div class="flex-1 overflow-y-auto space-y-1 max-h-48">
          <button
            ...
            :class="tab === 'dm' ? selectedDM === user.id ? 'bg-orange-500/10 border border-orange-500/30' : 'hover:bg-black/[0.04] dark:hover:bg-white/5' : selectedGroup.includes(user.id) ? 'bg-orange-500/10 border border-orange-500/30' : 'hover:bg-black/[0.04] dark:hover:bg-white/5'"
            @click="tab === 'dm' ? (selectedDM = user.id) : toggleGroup(user.id)"
          >
            <div class="size-9 rounded-xl bg-orange-500/10 flex items-center justify-center text-orange-400 font-semibold text-sm shrink-0">
        ...
        <!-- Confirm -->
        <button
          class="w-full py-3 rounded-2xl bg-orange-500 text-white font-semibold text-sm uppercase disabled:opacity-40 disabled:cursor-not-allowed hover:opacity-90 transition-opacity active:scale-[0.98]"
