...
<template>
  ...
  <div v-if="isMine" class="flex justify-end mb-1.5">
    <div class="relative group max-w-[75%]">
      <div class="bg-orange-500 text-white rounded-3xl rounded-br-sm px-4 py-2.5 text-sm">
  ...
  <div v-else class="flex justify-start mb-1.5 gap-2">
    <div
      ...
      class="w-7 h-7 rounded-full bg-orange-500/20 flex items-center justify-center text-[10px] font-black text-orange-400 shrink-0 mt-auto"
    >
    ...
    <div class="relative group max-w-[75%]">
      ...
      <div class="bg-black/[0.06] dark:bg-white/10 text-on-surface rounded-3xl rounded-bl-sm px-4 py-2.5 text-sm">
        <p class="break-words leading-relaxed">{{ message.isDeleted ? '[deleted]' : message.body }}</p>
      </div>
