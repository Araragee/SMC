...
<template>
  <button
    class="fixed top-4 left-4 z-[105] lg:hidden w-12 h-12 bg-surface-container-low/80 rounded-full border border-on-surface/[0.08] dark:border-on-surface/10 flex items-center justify-center shadow-lg"
    @click="toggleSidebar"
  >
  ...
  <aside
    class="fixed top-0 left-0 h-screen w-full glass-thin border-r border-on-surface/5 dark:border-on-surface/5 p-4 flex flex-col z-[100] lg:w-full lg:sticky lg:top-6 lg:h-auto lg:rounded-[2rem] lg:shadow-2xl lg:translate-x-0"
    :class="isSidebarOpen ? 'translate-x-0' : '-translate-x-full'"
  >
    <!-- Logo Section -->
    <div class="flex flex-col items-center py-4 mb-2">
      <div
        class="w-12 h-12 rounded-2xl bg-primary flex items-center justify-center shadow-lg shrink-0 mb-3 overflow-hidden"
      >
      ...
      <div class="text-center leading-none">
        ...
        <p class="text-primary text-[11px] font-bold uppercase tracking-wider">Music Clinic</p>
      </div>
    </div>
    ...
    <div class="py-3 border-on-surface/[0.04] dark:border-on-surface/5 flex flex-col gap-3 relative">
      <div class="flex flex-col items-center gap-2 w-full">
        ...
        >
          ...
          <span
            v-if="unreadCount > 0"
            class="absolute top-1.5 right-2 w-4 h-4 bg-primary rounded-full text-[9px] font-black text-on-surface flex items-center justify-center"
        ...
        >
          ...
          <span
            ...
            class="absolute top-1.5 right-2 min-w-[16px] h-4 bg-primary rounded-full text-[9px] font-black text-on-surface flex items-center justify-center px-0.5"
      ...
      <div class="relative w-full">
        <button
          class="w-full flex items-center gap-3 px-3 py-2.5 rounded-2xl hover:bg-on-surface/5 dark:hover:bg-on-surface/5 group border border-transparent"
          :class="{
            'bg-on-surface/10 border-on-surface/[0.08] dark:border-on-surface/10 shadow-lg': isUserDropdownOpen,
          }"
          @click.stop="toggleUserDropdown"
        >
          <div
            class="w-10 h-10 rounded-full bg-primary/20 border border-primary/30 flex items-center justify-center text-primary font-black text-sm shrink-0 group-hover:border-primary/50 overflow-hidden"
          >
            <img
        ...
        >
          ...
          <div class="p-2">
            <button
              class="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-on-surface-variant dark:text-on-surface-variant hover:text-on-surface dark:hover:text-on-surface hover:bg-on-surface/5 dark:hover:bg-on-surface/5 text-xs font-bold"
              @click="openSettings"
            >
