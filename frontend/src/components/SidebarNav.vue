...
<template>
  ...
  <button
    class="fixed top-4 left-4 z-[105] lg:hidden w-12 h-12 bg-surface-container-low/80 backdrop-blur-lg rounded-full border border-white/10 flex items-center justify-center shadow-lg"
  ...
  >
    ...
    <div class="flex flex-col items-center py-4 mb-2">
      <div class="w-12 h-12 rounded-2xl bg-gradient-to-br from-orange-500 to-orange-700 flex items-center justify-center shadow-lg shadow-orange-900/40 shrink-0 mb-3">
        <span class="material-symbols-outlined text-white text-2xl" style="font-variation-settings: 'FILL' 1">music_note</span>
      </div>
      <div class="text-center leading-none">
        <h2 class="text-white font-black tracking-tight text-lg">Sernan's</h2>
    ...
    <!-- Bottom Section -->
    <div class="pt-4 border-t border-white/5 flex flex-col gap-3 relative">
      <div class="flex items-center gap-3">
        <button
          @click="isNotificationsOpen = true"
          class="relative flex-1 flex items-center justify-center gap-2 py-3 bg-white/5 hover:bg-white/10 rounded-2xl text-zinc-400 hover:text-white transition-all group border border-white/5"
        >
      ...
      <div class="relative w-full">
        <button
          ...
          :class="{ 'bg-white/10 border-white/10 shadow-lg': isUserDropdownOpen }"
        >
          <div class="w-10 h-10 rounded-full bg-orange-500/20 border border-orange-500/30 flex items-center justify-center text-orange-400 font-black text-sm shrink-0 group-hover:border-orange-500/50 transition-colors overflow-hidden">
          ...
          <div class="flex-1 text-left min-w-0">
            <p class="text-sm font-bold text-white leading-tight truncate">{{ authStore.currentUser?.name || 'User' }}</p>
            <p class="text-[10px] text-zinc-500 font-bold uppercase tracking-widest truncate">{{ roleLabel }}</p>
          </div>
          <span
            class="material-symbols-outlined text-zinc-500 text-xl transition-transform duration-300"
        ...
        <Transition enter-active-class="transition-all duration-200 ease-out" enter-from-class="opacity-0 translate-y-2 scale-95 blur-[4px]" enter-to-class="opacity-100 translate-y-0 scale-100 blur-0" leave-active-class="transition-all duration-200 ease-in" leave-from-class="opacity-100 translate-y-0 scale-100 blur-0" leave-to-class="opacity-0 translate-y-2 scale-95 blur-[4px]">
          <div
            ...
            class="absolute bottom-full right-0 mb-3 w-56 liquid-glass rounded-[1.5rem] border border-white/10 shadow-2xl overflow-hidden z-[110]"
            v-click-outside="() => isUserDropdownOpen = false"
          >
            <div class="p-4 border-b border-white/5 bg-white/[0.02]">
              <p class="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-1">Signed in as</p>
              <p class="text-sm font-bold text-white truncate">{{ authStore.currentUser?.email }}</p>
            </div>

            <div class="p-2">
              <button
                @click="isSettingsOpen = true; isUserDropdownOpen = false"
                class="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-zinc-400 hover:text-white hover:bg-white/5 transition-all text-xs font-bold"
              >
                <span class="material-symbols-outlined text-lg">person_edit</span>
                Profile Settings
              </button>
              <button class="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-zinc-400 hover:text-white hover:bg-white/5 transition-all text-xs font-bold">
              ...
              <button
                @click="logout"
                class="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-zinc-500 hover:text-red-400 hover:bg-red-500/10 transition-all text-xs font-black"
              >
