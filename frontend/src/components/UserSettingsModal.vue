      ...
      >
        ...
        >
          ...
          <div class="p-8 pb-0 flex items-start justify-between">
            <div>
              <p class="text-[10px] font-black text-primary uppercase tracking-[0.3em] mb-2">
                Account Control
              </p>
              <h2 class="text-3xl font-black text-on-surface dark:text-on-surface tracking-tight">User Settings</h2>
            </div>
            <button
              class="w-12 h-12 rounded-2xl bg-on-surface/5 dark:bg-on-surface/5 hover:bg-on-surface/10 dark:hover:bg-on-surface/10 border border-on-surface/8 dark:border-on-surface/10 flex items-center justify-center text-on-surface-variant dark:text-on-surface-variant hover:text-on-surface transition-all group"
              @click="$emit('close')"
            >
              <span
            ...
            <section class="space-y-6">
              <div class="flex items-center gap-6">
                <div class="relative group">
                  <div
                    class="w-24 h-24 rounded-[2rem] bg-primary/10 border-2 border-dashed border-primary/30 flex items-center justify-center overflow-hidden transition-all group-hover:border-primary/60"
                  >
                    <img
                    ...
                    />
                    <span v-else class="text-3xl font-black text-primary/40">{{
            ...
            <section class="space-y-6">
              ...
              <div class="bg-surface-container-low dark:bg-surface-container-low border border-outline-variant dark:border-outline-variant rounded-3xl p-6 space-y-4">
                ...
                <div v-if="authStore.user?.totpEnabled" class="space-y-4">
                  ...
                  <div v-else class="space-y-4 pt-4 border-t border-outline-variant/10">
                    ...
                    <div class="flex gap-2 justify-end">
                      ...
                      <button
                        type="button"
                        class="px-4 py-2 bg-red-500 text-on-surface text-xs font-black rounded-xl uppercase tracking-widest hover:bg-red-600 transition-all"
                ...
                <div v-else>
                  <button
                    type="button"
                    class="w-full py-3 bg-primary/10 hover:bg-primary/20 border border-primary/20 text-primary font-black rounded-2xl text-xs uppercase tracking-widest transition-all"
                ...
                <div class="flex rounded-2xl bg-on-surface/[0.04] dark:bg-on-surface/5 border border-on-surface/[0.06] dark:border-on-surface/8 p-1 gap-1">
                  <button
                    ...
                    :class="themeStore.preference === opt.value
                      ? 'bg-surface-container-lowest dark:bg-on-surface/15 text-primary dark:text-primary shadow-sm border border-on-surface/[0.06] dark:border-on-surface/10'
                      : 'text-on-surface-variant hover:text-on-surface hover:bg-on-surface/[0.04] dark:hover:bg-on-surface/5'"
          ...
          >
            ...
            <button
              ...
              class="px-10 py-3 bg-primary text-on-primary dark:text-on-surface font-black rounded-2xl shadow-lg transition-all hover:scale-[1.02] active:scale-95 disabled:opacity-50 disabled:scale-100 flex items-center gap-2"
              @click="handleSave"
            >
              <span
