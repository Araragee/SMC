...
const form = ref({
  teacherId:
    props.userRole === 'teacher' ? (props.currentUserId as number | null) : (null as number | null),
  studentId:
    props.userRole === 'student' ? (props.currentUserId as number | null) : (null as number | null),
    ...
    >
      ...
      >
        ...
        <div class="modal-shell relative w-full max-w-md glass-heavy rounded-3xl shadow-2xl">
          <!-- Header -->
          <div class="flex items-center justify-between p-6 border-b border-on-surface/5">
            <div>
              <p class="text-xs font-semibold text-primary uppercase mb-1">
                {{ userRole === 'admin' ? 'Direct Schedule' : 'Propose Session' }}
              </p>
              <h3 class="text-xl font-semibold text-on-surface">New Session Request</h3>
            </div>
          ...
          <div class="p-6 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label class="block text-xs font-semibold text-on-surface-variant uppercase mb-2"
                  >Date</label
                >
                <input v-model="form.date" type="date" :min="todayStr" class="input" />
              </div>
              <div>
                <label class="block text-xs font-semibold text-on-surface-variant uppercase mb-2"
                  >Time</label
            ...
            <p v-if="isPastDateTime" class="text-xs font-semibold text-error -mt-2">
            ...
            <div v-if="busySlotsOnSelectedDate.length > 0" class="alert-error">
              <p class="text-xs font-semibold text-error uppercase mb-1 flex items-center gap-1">
              ...
              <p v-if="overlapsWithBusySlot" class="text-xs font-semibold text-error mt-2">
            ...
            <div v-if="instruments && instruments.length > 0">
              ...
              <BaseDropdown
                ...
                :options="[
                  { value: null, label: 'No specific instrument' },
            ...
            <div>
              <label class="block text-xs font-semibold text-on-surface-variant uppercase mb-2"
                >Notes
                <span class="text-on-surface-variant normal-case font-medium"
                  >(optional)</span
                ></label
              >
              <textarea
            ...
            <div v-if="userRole !== 'admin'" class="alert-info">
              <span class="material-symbols-outlined text-tertiary text-base mt-0.5">info</span>
              <p class="text-on-tertiary-container dark:text-tertiary text-xs leading-relaxed">
                <template v-if="userRole === 'student'"
                  >Your request will be sent to your teacher for review, then forwarded to admin for
                  final approval.</template
                >
                <template v-else
                  >Your proposal will be sent to admin for final approval before being
                  confirmed.</template
                >
              </p>
            </div>
          ...
          <!-- Footer -->
          <div class="p-4 border-t border-on-surface/5 flex gap-3">
            <button
              ...
              @click="submit"
            >
              <span v-if="isSubmitting" class="material-symbols-outlined text-base animate-spin"
                >refresh</span
              >
            ...
            <button
              class="px-4 py-3 bg-on-surface/5 hover:bg-on-surface/10 dark:hover:bg-on-surface/10 border border-on-surface/8 dark:border-on-surface/10 text-on-surface-variant hover:text-on-surface font-bold rounded-2xl transition-all text-sm"
              @click="$emit('close')"
            >
              Cancel
            </button>
