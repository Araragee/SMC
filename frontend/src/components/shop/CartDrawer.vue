  ...
  >
    ...
    <div class="pointer-events-none fixed inset-y-0 right-0 flex max-w-full pl-10">
      <div class="pointer-events-auto w-screen max-w-md transform transition-transform duration-500 ease-in-out">
        <div class="flex h-full flex-col bg-[#0e0e0e] border-l border-white/5 shadow-2xl shadow-black">
          <!-- Header -->
          <div class="p-8 border-b border-white/5 flex items-center justify-between">
            <h2 class="text-2xl font-black text-on-surface uppercase tracking-tight" id="slide-over-title">
              Shopping <span class="text-orange-500">Cart</span>
            </h2>
            <button
              @click="emit('close')"
              class="w-10 h-10 rounded-xl bg-white/5 text-on-surface-variant hover:text-white transition-all flex items-center justify-center"
            >
          ...
          <div class="flex-1 overflow-y-auto p-8 scrollbar-hide">
            <div v-if="shopStore.cart.length > 0" class="space-y-6">
              <div
                ...
                :key="item.productId"
                class="flex gap-4 p-4 rounded-3xl bg-white/5 border border-white/5"
              >
                <div class="w-20 h-20 rounded-2xl overflow-hidden bg-black/20 shrink-0">
                  <img
                    v-if="shopStore.getCartProduct(item.productId)?.imageUrl"
                    :src="shopStore.getCartProduct(item.productId)?.imageUrl"
                ...
                <div class="flex-1 min-w-0">
                  <h4 class="font-black text-on-surface truncate">{{ shopStore.getCartProduct(item.productId)?.name }}</h4>
                  <p class="text-sm font-black text-orange-500 mb-3">
                    {{ formatPrice(shopStore.getCartProduct(item.productId)?.priceCents || 0) }}
                  </p>

                  <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3 bg-black/20 rounded-xl px-2 py-1">
                      <button
                      ...
                      <span class="text-xs font-black text-on-surface w-4 text-center">{{ item.quantity }}</span>
                      <button
                        @click="shopStore.updateCartItem(item.productId, item.quantity + 1)"
                        class="text-on-surface-variant hover:text-white transition-colors"
                      >
              ...
              <!-- Notes -->
              <div class="pt-6">
                <label class="text-[10px] font-black text-on-surface-variant uppercase tracking-widest mb-3 block">Order Notes</label>
                <textarea
                  v-model="notes"
                  placeholder="Anything we should know? (e.g. delivery preferences)"
                  class="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-xs font-bold text-on-surface placeholder:text-on-surface-variant/50 focus:ring-2 focus:ring-orange-500/50 transition-all outline-none resize-none h-24"
                ></textarea>
              </div>
            </div>
          ...
          <!-- Footer -->
          <div v-if="shopStore.cart.length > 0" class="p-8 border-t border-white/5 bg-black/20">
            <div class="flex items-center justify-between mb-6">
              <span class="text-sm font-black text-on-surface-variant uppercase tracking-widest">Total Amount</span>
              <span class="text-2xl font-black text-on-surface tracking-tighter">{{ formatPrice(shopStore.cartTotalCents) }}</span>
            </div>
