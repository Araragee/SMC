...
<template>
  <div class="glass-thin rounded-[2.5rem] overflow-hidden border border-outline-variant/20 dark:border-on-surface/5 hover:border-primary/30 transition-all group flex flex-col h-full bg-surface-container-low/50 dark:bg-transparent">
    <!-- Image -->
    <div class="aspect-square relative overflow-hidden bg-on-surface/5 dark:bg-on-surface/40">
      ...
      <div v-if="product.category" class="absolute top-4 left-4">
        <span class="px-4 py-1.5 rounded-full bg-on-surface/40 dark:bg-on-surface/60 border border-on-surface/10 text-[10px] font-black uppercase tracking-widest text-on-surface">
    ...
    <div class="p-6 flex flex-col flex-grow">
      ...
      <div class="mt-auto flex items-center justify-between gap-4">
        <div>
          ...
          <p class="text-xl font-black text-primary tracking-tighter">{{ formatPrice(product.priceCents) }}</p>
        </div>

        <button
          @click="handleAddToCart"
          :disabled="product.stock <= 0"
          class="h-12 w-12 rounded-2xl bg-on-surface text-surface dark:bg-surface-container-lowest dark:text-on-surface hover:bg-primary hover:text-on-surface dark:hover:bg-primary dark:hover:text-on-surface disabled:opacity-20 transition-all flex items-center justify-center shadow-lg shadow-e2 dark:shadow-e2 active:scale-90"
        >
